...
<xsl:transform version="2.0"
    ...
    <xsl:template name="httpUrlReplacementSchemaDecl">
        ...
        <xsl:for-each select="//w11:message">
            ...
            <xsl:variable name="operation"
                          select="//w11:portType/w11:operation[resolve-QName(w11:input/@message, .) = $message_name]"/>
            <xsl:for-each select="$operation">
                ...
                <xsl:variable name="bound_to_http">
                    <xsl:for-each
                            select="//w11:binding[resolve-QName(@type, .) = $porttype_name]/w11:operation[@name = $operation_name]">
    ...
    <xsl:template name='resolve-elementType'>
        ...
        <xsl:variable name="bound_operation"
                      select="//w11:binding[resolve-QName(@type, current()) = $portType_name]/w11:operation[@name=$operation_name]"/>
        ...
        <xsl:variable name="soapbody"
                      select="$bound_operation/w11:input[resolve-QName(@message, current())=$message_name]/w11soap:body|$bound_operation/w11:output[resolve-QName(@message, current())=$message_name]/w11soap:body"/>
        <xsl:variable name="parts" select="tokenize($soapbody/@parts, '\s+')"/>
        <xsl:choose>
            <!-- Is this SOAP RPC? -->
        ...
        <xsl:element name='{local-name()}' namespace='http://www.w3.org/ns/wsdl'>
            ...
            <xsl:if test="$elementName">
                <wsoap:header required="true">
                    <xsl:variable name="element"
                                  select="resolve-QName($elementName, //w11:message[@name=$messageName]/w11:part[@name=$partName])"/>
                    <xsl:namespace name='{prefix-from-QName($element)}'
                                   select='namespace-uri-from-QName($element)'/>
                    <xsl:attribute name="element">
                        <xsl:value-of select="$element"/>
